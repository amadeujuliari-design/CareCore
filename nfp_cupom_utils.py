...
# Modelos fiscais aceitos na fila NFP (NF-e / NFC-e).
MODELOS_CHAVE_NFE_OK = frozenset({"55", "65"})


def digito_verificador_chave_nfe(base43: str) -> str:
    """Calcula o DV (mod 11) dos 43 primeiros digitos da chave de acesso."""
...
def validar_chave_acesso_nfe(chave: str) -> tuple[bool, str]:
    """Valida estrutura da chave de acesso (44 digitos).

    Retorna (ok, motivo). Motivo vazio quando ok.
    Nao consulta SEFAZ — so impede QR/digitacao corrompida de entrar na fila.
    """
    digitos = re.sub(r"\D", "", chave or "")
    if len(digitos) != 44:
    ...
    modelo = digitos[20:22]
    if modelo not in MODELOS_CHAVE_NFE_OK:
...
def mensagem_chave_invalida(motivo: str) -> str:
    ...
    if not m.lower().startswith("chave"):
        m = f"Chave invalida: {m}"
    return (m + " Removida da fila de envio SEFAZ.")[:2000]
