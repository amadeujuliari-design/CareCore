...
export default function Login() {
  ...
  const [revealAtivo, setRevealAtivo] = useState(() => !devePularRevealMarca());
  const [revealSaindo, setRevealSaindo] = useState(false);
  const [palcoExpandido, setPalcoExpandido] = useState(() => devePularRevealMarca());
  const [escalaInicial, setEscalaInicial] = useState({ x: 0.52, y: 0.38 });
  const loginRef = useRef(null);
  const palcoExpandidoRef = useRef(palcoExpandido);
  const DURACAO_TRANSICAO_MS = 5400;

  useEffect(() => {
    palcoExpandidoRef.current = palcoExpandido;
  }, [palcoExpandido]);

  useLayoutEffect(() => {
    if (devePularRevealMarca()) return undefined;

    const medir = () => {
      if (palcoExpandidoRef.current) return;
      const el = loginRef.current;
      if (!el) return;
      // offset* ignora transform — getBoundingClientRect incluía o scale e
      // fazia a escala "inicial" virar ~1 (sem animação de tamanho).
      const largura = el.offsetWidth;
      const altura = el.offsetHeight;
      if (!largura || !altura) return;
      const alvoW = Math.min(window.innerWidth * 0.92, 42 * 16);
      const alvoH = Math.min(window.innerHeight * 0.58, 24 * 16);
      setEscalaInicial({
        x: Math.max(0.28, Math.min(0.95, alvoW / largura)),
        y: Math.max(0.28, Math.min(0.95, alvoH / altura)),
      });
    };
  ...
  const concluirReveal = () => {
    if (!revealAtivo || revealSaindo) return;
    setRevealSaindo(true);
    // Garante paint no tamanho do quadro antes de expandir (scale + opacity juntos).
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        setPalcoExpandido(true);
      });
    });
    window.setTimeout(() => {
      setRevealAtivo(false);
      setRevealSaindo(false);
    }, DURACAO_TRANSICAO_MS + 200);
  };
  ...
  const loginLiberado = palcoExpandido || !revealAtivo;

  const liberarLoginCedo = () => {
    if (revealAtivo) concluirReveal();
  };

  const escalaAtual = palcoExpandido
    ? { x: 1, y: 1 }
    : escalaInicial;
  const animandoMorph = revealSaindo || (palcoExpandido && revealAtivo);

  return (
    <div
      className={`relative flex min-h-screen items-center justify-center overflow-hidden p-4 transition-colors ease-out ${
        revealAtivo && !palcoExpandido
          ? 'bg-gradient-to-br from-[#0b1f33] via-[#0f766e] to-[#b59a3c]'
          : 'bg-gradient-to-br from-sky-500 via-teal-500 to-amber-300'
      }`}
    ...
    >
      <div className="relative z-10 flex w-full max-w-6xl items-center justify-center">
        {revealAtivo && (
          <div
            className={`absolute z-20 flex flex-col items-center justify-center overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#0b1f33] via-[#0f766e] to-[#b59a3c] shadow-2xl transition-opacity ease-out ${
              revealSaindo ? 'pointer-events-none opacity-0' : 'opacity-100'
            }`}
            style={{
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Apresentação da marca CARECORE+"
          >
            <video
              ...
              playsInline
              preload="auto"
              onEnded={concluirReveal}
              onError={concluirReveal}
              aria-label="Animação da marca CARECORE+"
            >
            ...
            <button
              type="button"
              onClick={concluirReveal}
              className="absolute bottom-4 rounded-full border border-white/25 bg-black/20 px-5 py-2 text-sm font-semibold text-white backdrop-blur transition hover:bg-black/35"
            >
              Pular introdução
            </button>
          </div>
        ...
        <div
          ref={loginRef}
          className={`w-full origin-center overflow-hidden rounded-[2rem] border border-white bg-white shadow-2xl will-change-transform ${
            loginLiberado ? 'relative z-30' : ''
          }`}
          style={{
            transform: `scale(${escalaAtual.x}, ${escalaAtual.y})`,
            opacity: palcoExpandido ? 1 : 0,
            transitionProperty: animandoMorph ? 'transform, opacity' : 'none',
            transitionDuration: `${DURACAO_TRANSICAO_MS}ms`,
            transitionTimingFunction: 'cubic-bezier(0.22, 1, 0.36, 1)',
            pointerEvents: loginLiberado ? 'auto' : 'none',
          }}
          aria-hidden={!loginLiberado ? 'true' : undefined}
          onFocusCapture={liberarLoginCedo}
          onPointerDownCapture={liberarLoginCedo}
        >
          <div className="grid w-full overflow-hidden bg-white lg:grid-cols-[1.05fr_0.95fr]">
