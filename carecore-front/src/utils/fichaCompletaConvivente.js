...
function renderIdentificacao(c, { listaTecnicos, quartos, origensEncaminhamento }) {
  ...
  return secao('Identificação e situação no serviço', [
    ...
    linha('Data de inclusão', dataBr(c.data_inclusao)),
    linha('Data de inativação', dataBr(c.data_inativacao)),
    linha('Data de nova vinculação', dataBr(c.data_nova_vinculacao)),
    linha('Data de entrada/vinculação', dataBr(c.data_entrada)),
    linha('Prontuário da saúde', c.prontuario_saude),
    linha('Referência CAPS', c.acompanhamento_caps),
