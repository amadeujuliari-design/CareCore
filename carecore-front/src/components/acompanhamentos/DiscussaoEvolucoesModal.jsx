...
import { STATUS_EVOLUCAO_DISCUSSAO } from '../../config/acompanhamentosConfig';
import { PremiumButton } from '../PremiumUI';
import {
  criarEvolucaoDiscussaoHospitalar,
  obterDiscussaoHospitalar,
...
export default function DiscussaoEvolucoesModal({
  registro,
  somenteLeitura,
  onFechar,
  onAtualizado,
  ...
  const [form, setForm] = useState({
    status_evolucao: 'Internado',
    data_evolucao: dataHojeISO(),
    observacoes: '',
  });

  const encerrada = detalhe?.situacao_atual === 'Encerrado';

  const carregar = async () => {
    try {
      setLoading(true);
      setErro('');
      const dados = await obterDiscussaoHospitalar(registro.id);
      setDetalhe(dados);
    } catch (error) {
  ...
  const salvarEvolucao = async () => {
    ...
    try {
      setSalvando(true);
      setErro('');
      await criarEvolucaoDiscussaoHospitalar(registro.id, {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/45 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">Evoluções da discussão</h2>
            <p className="mt-1 text-sm text-slate-500">
              {registro.convivente_nome} · {registro.hospital_exibicao || registro.nome_hospital}
            </p>
          ...
          <>
            <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm">
              ...
              <p className="mt-1 text-slate-600">
                Discussão: {formatarData(detalhe?.data_discussao)}
                {detalhe?.data_prevista_entrada ? ` · Previsão: ${formatarData(detalhe.data_prevista_entrada)}` : ''}
              </p>
              {detalhe?.observacoes && (
              ...
              {(detalhe?.evolucoes || []).length === 0 ? (
                ...
                <ul className="space-y-2">
                  {detalhe.evolucoes.map(evolucao => (
                    <li key={evolucao.id} className="rounded-xl border border-violet-100 bg-violet-50/50 p-3 text-sm">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <span className="font-bold text-violet-900">{evolucao.status_evolucao}</span>
              ...
              <div className="mt-5 rounded-xl border border-slate-200 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  <label className="text-sm">
                    ...
                    >
                      {STATUS_EVOLUCAO_DISCUSSAO.map(status => (
            ...
            {encerrada && (
              <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                Esta discussão foi encerrada. Novas evoluções não podem ser registradas.
              </p>
            )}
          </>
        )}
