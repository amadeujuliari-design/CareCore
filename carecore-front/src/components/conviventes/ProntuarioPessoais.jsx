        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] text-slate-500">
              Campo operacional do status inativo. Ao reativar, é limpo e arquivado no histórico abaixo.
            </p>
          </div>
          <div>
          ...
          <div className="md:col-span-2 lg:col-span-3">
            <label className="block text-xs font-semibold text-gray-700 mb-1">Inativações no histórico</label>
            {(formData.inativacoes_anteriores || []).length === 0 ? (
              <p className="rounded-lg border border-dashed border-slate-200 bg-white px-3 py-2 text-xs text-slate-500">
                Nenhuma inativação arquivada até o momento.
              </p>
            ) : (
              <ul className="flex flex-wrap gap-2">
                {(formData.inativacoes_anteriores || []).map((dataItem) => {
                  const valor = String(dataItem).split('T')[0];
                  const [ano, mes, dia] = valor.split('-');
                  const rotulo = ano && mes && dia ? `${dia}/${mes}/${ano}` : valor;
                  return (
                    <li
                      key={valor}
            ...
            <p className="mt-1 text-[10px] text-slate-500">
              Datas preservadas quando o convivente é reativado (não cortam presença).
            </p>
          </div>
          <div>
